...
func parseOptions(args []string, now time.Time, out io.Writer) (options, error) {
	...
	f.SetOutput(out)
	f.BoolVar(&o.Persist, "persist", false, "Windows: save a one-time headless OS task, then exit (requires signed-in user)")
	f.BoolVar(&o.List, "list", false, "Windows: list this user's persistent jobs and saved results")
	f.StringVar(&o.Remove, "remove", "", "Windows: cancel an unstarted job or delete its saved results; refuses running jobs")
