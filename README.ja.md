...
### Windowsで未サインインでも実行する（未リリース）

```powershell
.\dist\agent-at.exe --persist --logon password --at 23:30 --prompt-file .\request.txt
```

`--logon password` は同じジョブを**非対話セッション**で実行するよう登録します。保守再起動後に
サインイン画面のまま指定時刻を迎えるような、サインアウト状態でも対象になります。既定は
`--logon interactive` で、上記の動作は変わりません。

登録時にこのWindowsアカウントのパスワードをコンソールで一度だけ入力します。入力経路は
コンソールだけで、フラグ・ファイル・環境変数・リダイレクト入力は拒否します。コマンドラインや
shell履歴に残しません。資格情報を保存するのはタスクスケジューラで、agent-atは保存しません。
ジョブ記録、ログ、エラーメッセージのいずれにも現れません。管理者権限・昇格・新規サービスは
引き続き不要ですが、そのアカウントに対しOSが非対話ログオンを許可している必要があり、
登録の成否がそれを示します。

**Windowsのパスワードを変更すると、保存済みのpassword logonタスクは無言で実行されなくなります。**
agent-at起動前に失敗するため、ジョブ記録・ログ・結果には理由が残らず、`--list` は予約のままに
見えます。パスワード変更後は対象ジョブを登録し直してください。

契約の他の項目は変わりません。最小権限、常にheadless、時刻トリガ1個、追い掛け実行なし、
スリープ解除なし、繰返しなし、再試行なしです。`--list` は該当ジョブを `password logon` と
表示します。Linux／macOSは `--logon password` を拒否します（systemd user managerでは
lingerの設定が別途必要です）。このモードのnative受入はまだありません。
